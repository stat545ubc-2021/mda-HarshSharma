...
We will use binary logistic regression for our classification task.

```{r, message=FALSE}
# Need to convert diagnosis values for "M" and "B" to 1 and 0 respectively
# So that glm model can except numerical data appropriate for logit regression
...
Using the below summary table from report 2, we save it to output folder as a csv.
...
We will save in 'Output' folder using functions 'saveRDS()' and 'readRDS()'
